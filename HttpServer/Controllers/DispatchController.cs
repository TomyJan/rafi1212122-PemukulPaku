        ...
        public static void AddHandlers(WebApplication app)
        {
            app.Map("/query_dispatch", (ctx) =>
            {
                QueryDispatch rsp = new()
                {
                    ...
                    RegionList = new Region[] {
                        new Region() {
                            Retcode = 0,
                            DispatchUrl = $"http://{Global.config.Gameserver.Host}:{Global.config.Http.HttpPort}/query_gateway",
            ...
            app.Map("/query_gateway", (ctx) =>
            {
                ...
                QueryGateway rsp = new()
                {
                    ...
                    AccountUrl = $"http://{Global.config.Gameserver.Host}:{Global.config.Http.HttpPort}/account",
                    AccountUrlBackup = $"http://{Global.config.Gameserver.Host}:{Global.config.Http.HttpPort}/account",
                    ...
                    Gateway = Gameserver,
                    IsDataReady = true,
                    OaserverUrl = $"http://{Global.config.Gameserver.Host}:{Global.config.Http.HttpPort}/oaserver",
                    ServerCurTime = Global.GetUnixInSeconds(),
                    ServerCurTimezone = 8,
                    ServerExt = new ServerExt()
                    {
                        CdkeyUrl = $"http://{Global.config.Gameserver.Host}:{Global.config.Http.HttpPort}/common",
                        MihoyoSdkEnv = "2"
                    }
                };
                ctx.Response.Headers.Add("Content-Type", "application/json");
            ...
            if (matches.Success)
            {
                ...
                switch (type)
                {
                    case "os":
                        ...
                        {
                            $"http://{Global.config.Gameserver.Host}:{Global.config.Http.HttpPort}/asset_bundle/overseas01/1.1",
                            $"http://{Global.config.Gameserver.Host}:{Global.config.Http.HttpPort}/asset_bundle/overseas01/1.1"
                        } : new string[]
                        {
                    ...
                    case "gf":
                        if (version.Contains("beta"))
                        {
                            return Global.config.UseLocalCache ? new string[]
                            {
                                $"https://{Global.config.Gameserver.Host}:{Global.config.Http.HttpsPort}/asset_bundle/beta_release/1.0",
                                $"https://{Global.config.Gameserver.Host}:{Global.config.Http.HttpsPort}/asset_bundle/beta_release/1.0"
                            } : new string[]
                            {
                        ...
                        return Global.config.UseLocalCache ? new string[]
                        {
                            $"https://{Global.config.Gameserver.Host}:{Global.config.Http.HttpsPort}/asset_bundle/android01/1.0",
                            $"https://{Global.config.Gameserver.Host}:{Global.config.Http.HttpsPort}/asset_bundle/android01/1.0"
                        } : new string[]
                        {
                    ...
                    case "global":
                        return Global.config.UseLocalCache ? new string[]
                        {
                            $"https://{Global.config.Gameserver.Host}:{Global.config.Http.HttpsPort}/asset_bundle/usa01/1.1",
                            $"https://{Global.config.Gameserver.Host}:{Global.config.Http.HttpsPort}/asset_bundle/usa01/1.1"
                        } : new string[]
                        {
            ...
            if (matches.Success)
            {
                ...
                switch (type)
                {
                    case "os":
                        return Global.config.UseLocalCache ? new string[]
                        {
                            $"{Global.config.Gameserver.Host}:{Global.config.Http.HttpPort}/com.miHoYo.bh3oversea",
                            $"{Global.config.Gameserver.Host}:{Global.config.Http.HttpPort}/com.miHoYo.bh3oversea"
                        } : new string[]
                        {
                    ...
                    case "gf":
                        if (version.Contains("beta"))
                        {
                            return Global.config.UseLocalCache ? new string[]
                            {
                                $"{Global.config.Gameserver.Host}:{Global.config.Http.HttpPort}/tmp/CGAudio",
                                $"{Global.config.Gameserver.Host}:{Global.config.Http.HttpPort}/tmp/CGAudio"
                            } : new string[]
                            {
                ...
                switch (type)
                {
                    ...
                    case "gf":
                        if (version.Contains("beta"))
                        {
                            return Global.config.UseLocalCache ? new string[]
                            {
                                $"{Global.config.Gameserver.Host}:{Global.config.Http.HttpPort}/tmp/beta",
                                $"{Global.config.Gameserver.Host}:{Global.config.Http.HttpPort}/tmp/beta"
                            } : new string[]
                            {
                        ...
                        return Global.config.UseLocalCache ? new string[]
                        {
                            $"{Global.config.Gameserver.Host}:{Global.config.Http.HttpPort}/tmp/Original",
                            $"{Global.config.Gameserver.Host}:{Global.config.Http.HttpPort}/tmp/Original"
                        } : new string[]
                        {
                    ...
                    case "global":
                        return Global.config.UseLocalCache ? new string[]
                        {
                            $"{Global.config.Gameserver.Host}:{Global.config.Http.HttpPort}/tmp/com.miHoYo.bh3global",
                            $"{Global.config.Gameserver.Host}:{Global.config.Http.HttpPort}/tmp/com.miHoYo.bh3global"
                        } : new string[]
                        {
